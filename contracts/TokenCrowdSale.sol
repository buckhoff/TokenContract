 ...
 * @dev Multi-tier presale contract for an ERC20 token with optimized variable sizes
 */
...
UUPSUpgradeable
{
    ...
    uint16 public constant MINOR_EMERGENCY_THRESHOLD = 1;
    uint16 public constant CRITICAL_EMERGENCY_THRESHOLD = 2;
    ...
    uint32 public minTimeBetweenPurchases = 1 hours;
    ...
    function getCurrentTier() public view returns (uint8) {
        ...
        uint96 tokensSold = uint96(totalTokensSold());
    ...
    function totalTokensSold() public view returns (uint96) {
        return uint96(PresaleTiers.calculateTotalTokensSold(tiers));
    }

    /**
     * @dev Get the number of tokens remaining in a specific tier
     * @param _tierId The tier ID to check
     * @return amount The number of tokens remaining in the tier
     */
    function tokensRemainingInTier(uint8 _tierId) public view returns (uint96 amount) {
        require(_tierId < tiers.length, "Invalid tier ID");
    ...
    /**
     * @dev Helper function to reset recovery approvals
     */
    function resetRecoveryApprovals() internal {
        // Reset approval count
        recoveryApprovalsCount = 0;

        // We don't need to clear the mapping as we check emergencyState first
    }
     ...
     * @param _stateValue The emergency state to set
     */
    ...
    function declareEmergency(EmergencyState _stateEnum) external onlyRole(Constants.EMERGENCY_ROLE) {
        setEmergencyState(uint8(_stateEnum));
    }
     ...
     */
    function pausePresale() external onlyRole(Constants.EMERGENCY_ROLE) {
        setEmergencyState(1); // Set to MINOR_EMERGENCY
    }
    ...
    function resumePresale() external onlyRole(DEFAULT_ADMIN_ROLE) {
        // Only allow resuming from MINOR_EMERGENCY
        require(emergencyState == EmergencyState.MINOR_EMERGENCY, "Cannot resume from critical state");
        setEmergencyState(0); // Set to NORMAL
    }
    ...
    function handleEmergencyPause() external onlyFromRegistry(Constants.STABILITY_FUND_NAME) {
        if (emergencyState == EmergencyState.NORMAL) {
            declareEmergency(EmergencyState.MINOR_EMERGENCY);
        }
    }
