    ...
    RegistryAwareUpgradeable
{
    ...
    uint96 public currentTier = 0;
    uint96 public tierCount;
    ...
    uint96 public constant PRICE_DECIMALS = 1e6;
    ...
    */
     ...
     */
    function setPresaleTimes(uint96 _start, uint96 _end) external onlyOwner {
    ...
    function claimableTokens(address _user) public view returns (uint96 claimable) {
        ...
        uint96 userTierCount = uint96(tierAmounts.length);
        
        for (uint96 tierId = 0; tierId < userTierCount; tierId++) {
    ...
    function setTierDeadline(uint256 _tier, uint96 _deadline) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function extendTier(uint96 _newDeadline) external onlyRole(DEFAULT_ADMIN_ROLE) {
