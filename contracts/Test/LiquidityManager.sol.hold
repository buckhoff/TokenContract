...
interface IUniswapV2Router {
    ...
    function getAmountsOut(uint96 amountIn, address[] calldata path)
    external view returns (uint96[] memory amounts);
}
...
UUPSUpgradeable
{
    ...
    uint96 public targetTokenPrice;
    ...
    // Price monitoring
    uint40 public lastPriceUpdateTime;
    ...
    ) external onlyRole(Constants.ADMIN_ROLE) returns (uint16) {
    ...
    function addLiquidityPhase(
        ...
        uint96 _targetPrice,
        uint40 _deadline
    ) external onlyRole(Constants.ADMIN_ROLE) returns (uint96) {
        ...
        uint96 phaseId = uint96(liquidityPhases.length);
    ...
    function createLiquidityAtTargetPrice(
        uint96 _tokenAmount,
        uint96 _stablecoinAmount
    ) public onlyRole(Constants.ADMIN_ROLE) nonReentrant {
        require(_tokenAmount > 0, "MultiDexLiquidityManager: zero token amount");
        require(_stablecoinAmount > 0, "MultiDexLiquidityManager: zero stablecoin amount");
        ...
        for (uint16 i = 0; i < dexes.length; i++) {
            if (dexes[i].active) {
                ...
                uint96 dexTokenAmount = uint96((_tokenAmount * dexes[i].allocationPercentage) / 10000);
                uint96 dexStablecoinAmount = uint96((_stablecoinAmount * dexes[i].allocationPercentage) / 10000);
     ...
     */
    function deployLiquidityInPhases(uint96[] calldata _phaseIds) external onlyRole(Constants.ADMIN_ROLE) nonReentrant {
        for (uint96 i = 0; i < _phaseIds.length; i++) {
    ...
    function swapWithPriceFloor(
    ...
    ) external nonReentrant returns (uint96 amountOut) {
        require(_dexId < dexes.length, "MultiDexLiquidityManager: invalid DEX ID");
        require(dexes[_dexId].active, "MultiDexLiquidityManager: DEX not active");
        ...
        (uint112 reserve0, uint112 reserve1, ) = _getReserves(dex.pair);

        // Determine token order in the pair
        address token0 = IUniswapV2Pair(dex.pair).token0();
        bool isToken0 = address(token) == token0;

        uint96 tokenReserve = uint96(isToken0 ? reserve0 : reserve1);
        uint96 stablecoinReserve = uint96(isToken0 ? reserve1 : reserve0);

        // Calculate current price
        uint96 currentPrice = (stablecoinReserve * 1e18) / tokenReserve;
        ...
        if (_isTokenToStable) {
            ...
            // Execute swap
            uint96[] memory amounts = IUniswapV2Router(dex.router).swapExactTokensForTokens(
                _amountIn,
                _minAmountOut,
                path,
                msg.sender,
            uint40(block.timestamp + 1800) // 30 minutes
            );
     ...
     */
    function stakeLPTokens(uint16 _dexId, uint96 _amount) external onlyRole(Constants.ADMIN_ROLE) nonReentrant {
        require(_dexId < dexes.length, "MultiDexLiquidityManager: invalid DEX ID");
        require(dexes[_dexId].active, "MultiDexLiquidityManager: DEX not active");
     ...
     * @param _dexId ID of the DEX
     */
    function claimLPRewards(uint16 _dexId) external onlyRole(Constants.ADMIN_ROLE) nonReentrant {
        require(_dexId < dexes.length, "MultiDexLiquidityManager: invalid DEX ID");
        require(dexes[_dexId].stakingRewards != address(0), "MultiDexLiquidityManager: no staking rewards");
     ...
     */
    function renounceLpOwnership(uint16 _dexId, address _deadAddress) external onlyRole(Constants.ADMIN_ROLE) nonReentrant {
        require(_dexId < dexes.length, "MultiDexLiquidityManager: invalid DEX ID");
        require(dexes[_dexId].active, "MultiDexLiquidityManager: DEX not active");
     ...
     * @param _deadAddress Address to send LP tokens to
     */
    function _renounceLpOwnership(uint16 _dexId, address _deadAddress) internal {
        DexInfo storage dex = dexes[_dexId];
        ...
        IUniswapV2Pair pair = IUniswapV2Pair(dex.pair);
        uint96 lpBalance = pair.balanceOf(address(this));
     ...
     */
    function updateTargetPrice(uint96 _newTargetPrice) external onlyRole(Constants.ADMIN_ROLE) {
     ...
     */
    function updatePriceFloor(uint96 _newPriceFloor) external onlyRole(Constants.ADMIN_ROLE) {
     ...
     */
    function getDexCount() external view returns (uint16 dexCount) {
     ...
     */
    function getLiquidityPhaseCount() external view returns (uint96 phaseCount) {
        return uint96(liquidityPhases.length);
    }
     ...
     */
    function getLpTokenBalance(uint16 _dexId) external view returns (uint96 balance) {
        require(_dexId < dexes.length, "MultiDexLiquidityManager: invalid DEX ID");
    ...
    function getDexReserves(uint16 _dexId) external view returns (
        ...
        if (dex.pair != address(0)) {
            ...
            // Calculate current price
            if (tokenReserve > 0) {
                currentPrice = uint96((stableReserve * 1e18) / tokenReserve);
            }
    ...
    function checkLiquidityHealth() external view returns (
        ...
        for (uint16 i = 0; i < dexes.length; i++) {
            if (dexes[i].active && dexes[i].pair != address(0)) {
                ...
                uint96 tokenReserve = uint96(isToken0 ? reserve0 : reserve1);
                uint96 stableReserve = uint96(isToken0 ? reserve1 : reserve0);
                ...
                // Calculate current price
                uint96 currentPrice = uint96((stableReserve * 1e18) / tokenReserve);

                // Check price deviation from target
                uint16 priceDeviation;
                if (currentPrice > targetTokenPrice) {
                    priceDeviation = uint16(((currentPrice - targetTokenPrice) * 10000) / targetTokenPrice);
                } else {
                    priceDeviation = uint16(((targetTokenPrice - currentPrice) * 10000) / targetTokenPrice);
                }
                ...
                if (tokenValue > stableReserve) {
                    imbalance = uint96(((tokenValue - stableReserve) * 10000) / tokenValue);
                } else {
                    imbalance = uint96(((stableReserve - tokenValue) * 10000) / stableReserve);
                }
    ...
    function _findNextPendingPhase() internal view returns (uint96) {
        for (uint96 i = 0; i < liquidityPhases.length; i++) {
    ...
    function performRebalancing() external onlyRole(Constants.ADMIN_ROLE) nonReentrant {
        ...
        DexPriceInfo[] memory dexInfos = new DexPriceInfo[](activeDexCount);
        uint16 infoIndex = 0;

        // Collect price and reserve info
        for (uint16 i = 0; i < dexes.length; i++) {
            if (dexes[i].active && dexes[i].pair != address(0)) {
                (uint112 reserve0, uint112 reserve1, ) = _getReserves(dexes[i].pair);
                ...
                // Calculate price
                uint96 price = uint96(tokenReserve > 0 ? (stableReserve * 1e18) / tokenReserve : 0);
            
                // Use the new struct name
                dexInfos[infoIndex] = DexPriceInfo({
                    dexId: i,
                    price: price,
        ...
        uint96 lowestDelta = type(uint96).max;
        uint96 highestDelta = type(uint96).max;

        for (uint16 i = 0; i < dexInfos.length; i++) {
            if (dexInfos[i].price < targetTokenPrice) {
                uint96 delta = targetTokenPrice - dexInfos[i].price;
                if (delta < lowestDelta) {
                    lowestDelta = delta;
                    lowestPriceDexIndex = i;
                }
            } else {
                uint96 delta = dexInfos[i].price - targetTokenPrice;
        ...
        if (dexInfos[highestPriceDexIndex].price > dexInfos[lowestPriceDexIndex].price) {
            divergence = uint16(((dexInfos[highestPriceDexIndex].price - dexInfos[lowestPriceDexIndex].price) * 10000) /
        ...
        if (divergence > maxPriceDivergence) {
            ...
            // DEX with higher price - we want to sell tokens here to reduce price
            DexPriceInfo memory highPriceDex = dexInfos[highestPriceDexIndex];
            // DEX with lower price - we want to buy tokens here to increase price
            ...
            uint96 swapAmount = uint96((priceGap * highPriceDex.tokenReserve) / (highPriceDex.price * 4)); // 25% impact

            // Cap swap amount
            uint96 maxSwap = uint96(highPriceDex.tokenReserve / 10); // Max 10% of reserves
            if (swapAmount > maxSwap) {
                swapAmount = maxSwap;
            }

            // Execute swaps if amount is significant
            if (swapAmount > 1e18) { // At least 1 token
                // 1. Swap tokens to stablecoin on high price DEX
                token.approve(dexes[highPriceDex.dexId].router, swapAmount);

                address[] memory path1 = new address[](2);
                path1[0] = address(token);
                path1[1] = address(stablecoin);

                uint96[] memory amounts1 = IUniswapV2Router(dexes[highPriceDex.dexId].router).swapExactTokensForTokens(
                    swapAmount,
                    0, // Accept any output amount
                    path1,
                    address(this),
                uint40(block.timestamp + 300) // 5 minute deadline
                );

                // 2. Swap stablecoin back to tokens on low price DEX
                stablecoin.approve(dexes[lowPriceDex.dexId].router, amounts1[1]);

                address[] memory path2 = new address[](2);
                path2[0] = address(stablecoin);
                path2[1] = address(token);

                IUniswapV2Router(dexes[lowPriceDex.dexId].router).swapExactTokensForTokens(
                    amounts1[1],
                    0, // Accept any output amount
                    path2,
                    address(this),
    ...
    function _addLiquidityToDex(
        ...
        // Add liquidity
        (uint96 tokenAmountAdded, uint96 stablecoinAmountAdded, uint96 liquidity) =
     ...
     */
    function _getReserves(address _pair) internal view returns (uint112 reserve0, uint112 reserve1, uint40 blockTimestampLast) {
        (uint112 _reserve0, uint112 _reserve1, uint40 _blockTimestampLast) =
                                IUniswapV2Pair(_pair).getReserves();
        return (uint112(_reserve0), uint112(_reserve1), _blockTimestampLast);
    }
