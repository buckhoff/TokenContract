...
contract TeachTokenPresale is Ownable, ReentrancyGuard {
    ...
    struct PresaleTier {
        uint256 price;        // Price in USD (scaled by 1e6)
        ...
        uint256 vestingTGE;   // Percentage released at Token Generation Event (scaled by 100)
        uint256 vestingMonths; // Remaining vesting period in months
    ...
    constructor(IERC20 _paymentToken, address _treasury) {
        ...
        // Tier 1: Strategic Private Investment Round
        tiers.push(PresaleTier({
            price: 18000, // $0.018
            allocation: 75_000_000 * 10**18, // 75M tokens
            sold: 0,
            minPurchase: 50_000 * PRICE_DECIMALS, // $50,000 min
            maxPurchase: 500_000 * PRICE_DECIMALS, // $500,000 max
            vestingTGE: 10, // 10% at TGE
            vestingMonths: 18, // 18 months vesting
            isActive: false
        }));

        // Tier 2: Institutional Private Round
        tiers.push(PresaleTier({
            price: 25000, // $0.025
            allocation: 100_000_000 * 10**18, // 100M tokens
            sold: 0,
            minPurchase: 25_000 * PRICE_DECIMALS, // $25,000 min
            maxPurchase: 250_000 * PRICE_DECIMALS, // $250,000 max
            vestingTGE: 15, // 15% at TGE
            vestingMonths: 15, // 15 months vesting
            isActive: false
        }));

        // Tier 3: Early Supporter Round
        tiers.push(PresaleTier({
            price: 35000, // $0.035
            allocation: 100_000_000 * 10**18, // 100M tokens
            sold: 0,
            minPurchase: 10_000 * PRICE_DECIMALS, // $10,000 min
            maxPurchase: 100_000 * PRICE_DECIMALS, // $100,000 max
            vestingTGE: 20, // 20% at TGE
            vestingMonths: 12, // 12 months vesting
            isActive: false
        }));

        // Tier 4: Community Leaders Round
        tiers.push(PresaleTier({
            price: 50000, // $0.05
            allocation: 75_000_000 * 10**18, // 75M tokens
            sold: 0,
            minPurchase: 5_000 * PRICE_DECIMALS, // $5,000 min
        ...
        // Tier 5: Early Public Round
        tiers.push(PresaleTier({
            price: 70000, // $0.07
            allocation: 50_000_000 * 10**18, // 50M tokens
            sold: 0,
            minPurchase: 1_000 * PRICE_DECIMALS, // $1,000 min
        ...
        // Tier 6: Main Public Round
        tiers.push(PresaleTier({
            price: 90000, // $0.09
            allocation: 50_000_000 * 10**18, // 50M tokens
            sold: 0,
            minPurchase: 500 * PRICE_DECIMALS, // $500 min
        ...
        // Tier 7: Final Opportunity Round
        tiers.push(PresaleTier({
            price: 110000, // $0.11
            allocation: 50_000_000 * 10**18, // 50M tokens
            sold: 0,
     ...
     */
    function purchase(uint256 _tierId, uint256 _usdAmount) external nonReentrant {
        ...
        // Update tier data
        tier.sold = tier.sold.add(tokenAmount);

        // Update user purchase data
        Purchase storage userPurchase = purchases[msg.sender];
        userPurchase.tokens = userPurchase.tokens.add(tokenAmount);
        userPurchase.usdAmount = userPurchase.usdAmount.add(_usdAmount);
    ...
    mapping(uint256 => uint256) public tierDeadlines; // Timestamps for tier deadlines
